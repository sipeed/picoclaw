...
> **Có gì mới?** Piconomous hiện sử dụng cách tiếp cận cấu hình **tập trung vào mô hình**. Chỉ cần chỉ định định dạng `vendor/model` (ví dụ: `zhipu/glm-4.7`) để thêm provider mới — **không cần thay đổi code!**
...
> Chạy `piconomous auth login --provider anthropic` để dán API token.
...
Piconomous chỉ loại bỏ tiền tố ngoài `litellm/` trước khi gửi yêu cầu, nên alias proxy như `litellm/lite-gpt4` gửi `lite-gpt4`, trong khi `litellm/openai/gpt-4o` gửi `openai/gpt-4o`.

#### Cân Bằng Tải

Cấu hình nhiều endpoint cho cùng tên mô hình — Piconomous sẽ tự động round-robin giữa chúng:
...
Piconomous định tuyến provider theo họ giao thức:
