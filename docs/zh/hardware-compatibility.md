...
# 🖥️ Piconomous 硬件兼容性列表

Piconomous 几乎可以在任何 Linux 设备上运行。本页面记录了已验证的芯片、产品和开发板。
...
已通过 Piconomous 测试的消费产品、路由器和工业设备。
...
任何 ARM64 Android 手机（2015 年以后），1GB 以上内存。安装 [Termux](https://github.com/termux/termux-app)，使用 `proot` 运行 Piconomous。
