...
Piconomous のツール設定は `config.json` の `tools` フィールドにあります。
...
デフォルトで、Piconomous は以下の危険なコマンドをブロックします：
...
exec ガードは Piconomous に送信されたトップレベルのコマンドのみを検証します。そのコマンドの実行開始後にビルドツールやスクリプトが生成する子プロセスを再帰的に検査することは**ありません**。
...
すべての設定オプションは `PICONOMOUS_TOOLS_<SECTION>_<KEY>` 形式の環境変数で上書きできます：
