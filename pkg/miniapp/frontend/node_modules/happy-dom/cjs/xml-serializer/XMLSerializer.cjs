...
class XMLSerializer {
    ...
    #getNamespacePrefixes(element, inheritedNamespacePrefixes) {
        ...
        const namedItems = element[PropertySymbol.attributes][PropertySymbol.namedItems];
        for (const attributes of namedItems.values()) {
            if (attributes[0][PropertySymbol.namespaceURI] === NamespaceURI_js_1.default.xmlns &&
                attributes[0][PropertySymbol.prefix]) {
                namespacePrefixes.set(attributes[0][PropertySymbol.value], attributes[0][PropertySymbol.localName]);
            }
        }
