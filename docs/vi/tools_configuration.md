...
Cấu hình công cụ của Piconomous nằm trong trường `tools` của `config.json`.
...
Theo mặc định, Piconomous chặn các lệnh nguy hiểm sau:
...
Bộ bảo vệ exec chỉ xác thực lệnh cấp cao nhất được gửi đến Piconomous. Nó **không** kiểm tra đệ quy các tiến trình con được tạo bởi các công cụ build hoặc script sau khi lệnh đó bắt đầu chạy.
...
Tất cả các tùy chọn cấu hình có thể được ghi đè qua biến môi trường với định dạng `PICONOMOUS_TOOLS_<SECTION>_<KEY>`:
