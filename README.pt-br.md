...
<h3>Hardware de $10 · 10MB de RAM · Boot em 1s · Vai Lacraia-do-mar!</h3>
...
> * **DOMINIO OFICIAL:** O **ÚNICO** site oficial é **[picoclaw.io](https://picoclaw.io)**, e o site da empresa é **[sipeed.com](https://sipeed.com)**.
...
🤖 **Auto-Construido por IA**: Implementação nativa em Go de forma autônoma — 95% do núcleo gerado pelo Agente com refinamento humano no loop.
...
Você também pode rodar o PicoClaw usando Docker Compose sem instalar nada localmente.
...
> Busca web é **opcional** — obtenha a [Brave Search API](https://brave.com/search/api) gratuita (2000 consultas grátis/mês) ou use o fallback automático integrado.
...
Em seguida, configure a Webhook URL no LINE Developers Console para `https://seu-domínio/webhook/line` e habilite **Use webhook**.
...
├── HEARTBEAT.md      # Prompts de tarefas periódicas (verificado a cada 30 min)
...
O PicoClaw roda em um ambiente sandbox por padrão. O agente só pode acessar arquivos e executar comandos dentro do workspace configurado.
...
Todos os caminhos compartilham a mesma restrição de workspace — não há como contornar o limite de segurança por meio de subagentes ou tarefas agendadas.
...
- Revisar minha agenda para próximos eventos
...
Isso acontece quando outra instância do bot está rodando. Certifique-se de que apenas um `picoclaw gateway` esteja rodando por vez.
