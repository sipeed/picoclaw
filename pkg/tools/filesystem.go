...
// validatePathWithAllowPaths ensures the given path is within the workspace if
// restrict is true. patterns optionally allow access to paths outside the
// workspace (used by shell.go for working directory validation).
...
// validatePath is a convenience wrapper around validatePathWithAllowPaths with
// no extra allow-path patterns. Used by shell.go and tests.
