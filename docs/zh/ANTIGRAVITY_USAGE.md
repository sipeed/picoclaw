...
# 在 Piconomous 中使用 Antigravity 提供商

本指南介绍如何在 Piconomous 中设置和使用 **Antigravity**（Google Cloud Code Assist）提供商。
...
5.  **从浏览器地址栏复制该最终 URL**。
6.  **将其粘贴回 Piconomous 正在等待的终端中**。

Piconomous 将自动提取授权码并完成流程。
...
### 切换模型
你可以在 `~/.piconomous/config.json` 中更改默认模型，或通过 CLI 覆盖：
...
*   **429 速率限制**：Antigravity 有严格的配额限制。如果触发限制，Piconomous 将在错误消息中显示"重置时间"。
*   **404 未找到**：确保你使用的是 `piconomous auth models` 列表中的模型 ID。请使用短 ID（例如 `gemini-3-flash`），而非完整路径。
