...
func (a *AuthProvider) VerifyToken(token *AuthToken) bool {
	...
	// Clear signature before verification so the HMAC input matches
	// what GenerateToken computed (with Signature as empty string).
	sig := token.Signature
	token.Signature = ""
	result := a.VerifySignature(token, sig)
	token.Signature = sig
	return result
}
...
func (a *AuthProvider) AuthenticateNode(nodeID, signature string, challengeData any) bool {
	if a.sharedSecret == nil {
		// No authentication configured - fail closed
		return false
	}
