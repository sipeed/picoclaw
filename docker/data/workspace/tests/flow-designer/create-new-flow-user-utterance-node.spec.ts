...
test('Create new flow with User Utterance node', async ({ page }) => {
  ...
  await page.locator('.modal-dialog .field-container').first().waitFor({ state: 'visible', timeout: 5000 });
  ...
  await saveDialog.waitFor({ state: 'hidden', timeout: 10000 });
  ...
  await page.locator('.node-container#START').waitFor({ state: 'visible', timeout: 5000 });
  await page.locator('.node-container').filter({ hasText: /ReplyMessage/ }).first().waitFor({ state: 'visible', timeout: 5000 });
  await page.locator('.node-container#input').waitFor({ state: 'visible', timeout: 5000 });
  await page.locator('.node-container#Output').waitFor({ state: 'visible', timeout: 5000 });
  await page.locator('.node-container#END').waitFor({ state: 'visible', timeout: 5000 });
