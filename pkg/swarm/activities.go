...
func NewActivities(provider providers.LLMProvider, agentLoop *agent.AgentLoop, cfg *config.SwarmConfig) *Activities {
	return &Activities{
		provider:  provider,
		agentLoop: agentLoop,
		cfg:       cfg,
...
Keep subtasks focused and independently executable. Each subtask should produce a partial result that can be synthesized later.`,
	...
	// Use default model since SwarmConfig doesn't have model config
	model := "gpt-4"
	if a.cfg != nil && len(a.cfg.Temporal.TaskQueue) > 0 {
		// In production, this would come from a proper config field
		model = "gpt-4"
	}
...
Provide a comprehensive synthesis that directly addresses the original task.`, resultsBlock.String())
	...
	response, err := a.provider.Chat(ctx, messages, nil, "gpt-4", map[string]interface{}{
		"max_tokens":  4096,
		"temperature": 0.5,
	})
	if err != nil {
		logger.WarnCF("swarm", "LLM synthesis failed, concatenating results", map[string]interface{}{
			"error": err.Error(),
		})
		// Fall back to concatenation
		var fallback strings.Builder
		for i, result := range results {
			fallback.WriteString(fmt.Sprintf("=== Result %d ===\n%s\n\n", i+1, result))
		}
