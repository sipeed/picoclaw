...
func TestClassifyCommand(t *testing.T) {
	tests := []struct {
		...
		{name: "git config get read only", command: "git config user.name", want: CommandClassReadOnly},
		{name: "git config list read only", command: "git config --list", want: CommandClassReadOnly},
		{name: "git config set write", command: "git config user.name Anton", want: CommandClassWrite},
		{name: "git config unset write", command: "git config --unset user.name", want: CommandClassWrite},
		{name: "git branch list read only", command: "git branch --list", want: CommandClassReadOnly},
		{name: "git branch contains read only", command: "git branch --contains HEAD", want: CommandClassReadOnly},
		{name: "git branch merged read only", command: "git branch --merged main", want: CommandClassReadOnly},
		{name: "git branch no-merged read only", command: "git branch --no-merged main", want: CommandClassReadOnly},
		{name: "git branch points-at read only", command: "git branch --points-at HEAD", want: CommandClassReadOnly},
		{
			name:    "git branch format read only",
			command: "git branch --format %(refname:short)",
			want:    CommandClassReadOnly,
		},
		{
			name:    "git branch contains equals read only",
			command: "git branch --contains=HEAD",
			want:    CommandClassReadOnly,
		},
		{name: "git branch create write", command: "git branch feature/test", want: CommandClassWrite},
		{name: "git branch delete write", command: "git branch -D feature/test", want: CommandClassWrite},
		{name: "git remote show read only", command: "git remote show origin", want: CommandClassReadOnly},
		{
			name:    "git remote add write",
			command: "git remote add fork git@github.com:owner/repo.git",
			want:    CommandClassWrite,
		},
...
func TestValidator_ReadOnlyPermissionMode(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "blocks mutating git config",
			command:       "git config user.name Anton",
			allowed:       false,
			commandClass:  CommandClassWrite,
			wantCategory:  "permission_mode",
			wantReasonSub: "read_only",
