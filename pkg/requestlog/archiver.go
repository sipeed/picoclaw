...
func (a *Archiver) Archive() error {
	return a.archiveWithCutoff(time.Now().AddDate(0, 0, -a.config.RetentionDays), false)
}

func (a *Archiver) ArchiveAll() error {
	return a.archiveWithCutoff(time.Now(), true)
}

func (a *Archiver) archiveWithCutoff(cutoff time.Time, excludeToday bool) error {
	...
	for _, file := range files {
		...
		if excludeToday && strings.Contains(name, today) {
			continue
		}
	...
	logger.InfoCF("requestlog.archiver", "Archive completed", map[string]any{
