...
你可以使用环境变量覆盖默认路径。这对于便携安装、容器化部署或将 piconomous 作为系统服务运行非常有用。这些变量是独立的，控制不同的路径。

| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `PICONOMOUS_CONFIG` | 覆盖配置文件的路径。这直接告诉 piconomous 加载哪个 `config.json`，忽略所有其他位置。 | `~/.piconomous/config.json` |
| `PICONOMOUS_HOME`   | 覆盖 piconomous 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.piconomous`             |

**示例：**

```bash
# 使用特定的配置文件运行 piconomous
# 工作区路径将从该配置文件中读取
PICONOMOUS_CONFIG=/etc/piconomous/production.json piconomous gateway

# 在 /opt/piconomous 中存储所有数据运行 piconomous
# 配置将从默认的 ~/.piconomous/config.json 加载
# 工作区将在 /opt/piconomous/workspace 创建
...
Piconomous 将数据存储在您配置的工作区中（默认：`~/.piconomous/workspace`）：
...
1. `~/.piconomous/workspace/skills`（工作区）
2. `~/.piconomous/skills`（全局）
3. `<构建时嵌入路径>/skills`（内置）
...
Piconomous 默认在沙箱环境中运行。Agent 只能访问配置的工作区内的文件和执行命令。
...
| 选项                    | 默认值                  | 描述                          |
| ----------------------- | ----------------------- | ----------------------------- |
| `workspace`             | `~/.piconomous/workspace` | Agent 的工作目录              |
...
exec 安全守卫仅检查 Piconomous 直接启动的命令行。它不会递归检查由 `make`、`go run`、`cargo`、`npm run` 或自定义构建脚本等开发工具产生的子进程。
...
* 如果需要比内置守卫更强的隔离，请在容器或虚拟机中运行 Piconomous。
...
Piconomous 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `PICONOMOUS_HEARTBEAT_ENABLED=false` 禁用
- `PICONOMOUS_HEARTBEAT_INTERVAL=60` 更改间隔
