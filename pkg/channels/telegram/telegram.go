...
func (c *TelegramChannel) Send(ctx context.Context, msg bus.OutboundMessage) error {
	...
	sChatID, err := parseChatID(msg.ChatID)
	if err != nil {
	...
	tgMsg := tu.Message(sChatID, htmlContent)
...
func (c *TelegramChannel) EditMessage(ctx context.Context, chatID string, messageID string, content string) error {
	...
	editMsg := tu.EditMessageText(cid, mid, htmlContent)
...
func (c *TelegramChannel) SendPlaceholder(ctx context.Context, chatID string) (string, error) {
	...
	pMsg, err := c.bot.SendMessage(ctx, tu.Message(cid, text))
	if err != nil {
		return "", err
	}
...
func parseChatID(chatIDStr string) (telego.ChatID, error) {
	...
	return telego.ChatID{}, fmt.Errorf("invalid chat ID: %s", chatIDStr)
}
