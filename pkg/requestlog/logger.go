...
type Logger struct {
	...
	pendingReqs map[string]*RequestRecord
...
func NewLogger(cfg Config, msgBus *bus.MessageBus, workspacePath string) *Logger {
	...
	return &Logger{
		...
		pendingReqs: make(map[string]*RequestRecord),
	}
}
...
func (l *Logger) consumeLoop() {
	...
	l.flushTicker = time.NewTicker(30 * time.Second)

	inboundCh := make(chan bus.InboundMessage, 10)
	outboundCh := make(chan bus.OutboundMessage, 10)

	go func() {
		for {
			msg, ok := l.msgBus.ConsumeInbound(l.ctx)
			if !ok {
				close(inboundCh)
				return
			}
	...
	go func() {
		for {
			msg, ok := l.msgBus.SubscribeOutbound(l.ctx)
			if !ok {
				close(outboundCh)
				return
			}
...
func (l *Logger) flushPending() {
	...
	for _, record := range l.pendingReqs {
		l.writeRecord(*record)
	}
	l.pendingReqs = make(map[string]*RequestRecord)
}
...
func (l *Logger) recordInbound(msg bus.InboundMessage) {
	...
	sessionKey := msg.SessionKey
	l.pendingMu.Lock()
	l.pendingReqs[sessionKey] = &record
	l.pendingMu.Unlock()

	l.writeRecord(record)
}

func (l *Logger) recordOutbound(msg bus.OutboundMessage) {
	sessionKey := msg.Channel + ":" + msg.ChatID

	l.pendingMu.Lock()
	record, ok := l.pendingReqs[sessionKey]
	if ok {
		delete(l.pendingReqs, sessionKey)
	}
	l.pendingMu.Unlock()
	...
	if l.config.LogContentMaxLength > 0 && len(response) > l.config.LogContentMaxLength {
		response = response[:l.config.LogContentMaxLength]
	}

	record.Response = response
	record.ResponseLength = len(msg.Content)
	record.ProcessingTime = time.Now().UTC().Sub(record.Timestamp).Milliseconds()
