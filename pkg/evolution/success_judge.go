...
func (j *HeuristicSuccessJudge) JudgeTaskRecord(
	...
	if strings.TrimSpace(record.UserGoal) == "" {
		return TaskSuccessDecision{Success: false, Reason: "missing user goal"}, nil
	}
	...
	if len(record.ToolExecutions) > 0 {
		...
		if allFailed {
			return TaskSuccessDecision{Success: false, Reason: "all tools failed"}, nil
		}
	}
	if strings.TrimSpace(record.FinalOutput) == "" && len(record.ToolExecutions) == 0 {
...
func buildTaskSuccessJudgePrompt(record LearningRecord) string {
	lines := []string{
		...
		"",
		"User goal: " + fallbackString(record.UserGoal, "none"),
		...
		"Tool kinds: " + joinOrFallback(record.ToolKinds, "none"),
		"Tool executions:",
	}
	if len(record.ToolExecutions) == 0 {
		...
		for _, exec := range record.ToolExecutions {
			...
			line := "- " + exec.Name + ": " + status
			if errSummary := strings.TrimSpace(exec.ErrorSummary); errSummary != "" {
				line += " (" + errSummary + ")"
			}
			if len(exec.SkillNames) > 0 {
				line += " [skills: " + strings.Join(exec.SkillNames, ", ") + "]"
			}
