...
func TestSecureStringRejectsFilteredPlaceholder(t *testing.T) {
	t.Run("fromRaw rejects FILTERED", func(t *testing.T) {
		s := NewSecureString("real-secret-value")
		assert.Equal(t, "real-secret-value", s.String())

		// fromRaw should not overwrite the real value with [FILTERED]
		err := s.fromRaw("[FILTERED]")
		assert.NoError(t, err)
		assert.Equal(t, "real-secret-value", s.String(), "[FILTERED] should not overwrite real value")
	})

	t.Run("fromRaw accepts real values", func(t *testing.T) {
		s := NewSecureString("old-value")
		err := s.fromRaw("new-value")
		assert.NoError(t, err)
		assert.Equal(t, "new-value", s.String())
	})

	t.Run("UnmarshalJSON rejects FILTERED", func(t *testing.T) {
		s := NewSecureString("real-secret")
		err := s.UnmarshalJSON([]byte(`"[FILTERED]"`))
		assert.NoError(t, err)
		assert.Equal(t, "real-secret", s.String(), "[FILTERED] via JSON should not overwrite real value")
	})

	t.Run("UnmarshalYAML rejects FILTERED", func(t *testing.T) {
		s := NewSecureString("real-secret")
		node := &yaml.Node{Value: "[FILTERED]"}
		err := s.UnmarshalYAML(node)
		assert.NoError(t, err)
		assert.Equal(t, "real-secret", s.String(), "[FILTERED] via YAML should not overwrite real value")
	})

	t.Run("UnmarshalText rejects FILTERED", func(t *testing.T) {
		s := NewSecureString("real-secret")
		err := s.UnmarshalText([]byte("[FILTERED]"))
		assert.NoError(t, err)
		assert.Equal(t, "real-secret", s.String(), "[FILTERED] via env var should not overwrite real value")
	})

	t.Run("UnmarshalText accepts real values", func(t *testing.T) {
		s := NewSecureString("old-value")
		err := s.UnmarshalText([]byte("new-value"))
		assert.NoError(t, err)
		assert.Equal(t, "new-value", s.String())
	})

	t.Run("MarshalYAML does not write FILTERED", func(t *testing.T) {
		s := SecureString{resolved: "[FILTERED]", raw: "[FILTERED]"}
		result, err := s.MarshalYAML()
		assert.NoError(t, err)
		assert.Equal(t, "", result, "[FILTERED] should not be written to YAML")
	})

	t.Run("empty SecureString fromRaw FILTERED stays empty", func(t *testing.T) {
		s := &SecureString{}
		err := s.fromRaw("[FILTERED]")
		assert.NoError(t, err)
		assert.Equal(t, "", s.String(), "empty SecureString should stay empty after [FILTERED]")
	})

	t.Run("env.Parse skips FILTERED env var", func(t *testing.T) {
		type TestSettings struct {
			Token SecureString `env:"TEST_SECURE_STRING_TOKEN"`
		}

		// Set up a real value first
		settings := &TestSettings{Token: *NewSecureString("real-token")}

		// Set env var to [FILTERED]
		t.Setenv("TEST_SECURE_STRING_TOKEN", "[FILTERED]")

		// env.Parse should not overwrite the real value with [FILTERED]
		err := env.Parse(settings)
		assert.NoError(t, err)
		assert.Equal(t, "real-token", settings.Token.String(),
			"env.Parse should not overwrite real value with [FILTERED]")
	})
}
>>>>>>> Stashed changes
