...
func NewDiscoveryService(cfg *Config) (*DiscoveryService, error) {
	...
	if advPort == 0 {
		advPort = cfg.BindPort //nolint:wastedassign // advPort reserved for future advertise logic
	}
...
func (ds *DiscoveryService) Join(ctx context.Context, addrs []string) (int, error) {
	...
	// Warn if no addresses could be reached
	if count == 0 && len(addrs) > 0 {
		logger.WarnCF(
			"swarm",
			"Failed to join cluster: no nodes were reachable. Node starting in isolated mode",
			map[string]any{
				"attempted_addrs": addrs,
				"local_node_id":   ds.localNode.ID,
			},
		)
	}
...
func (ds *DiscoveryService) handleGossip(data []byte, addr *net.UDPAddr) {
	...
	if ds.config.Discovery.RequireAuth {
		if msg.AuthToken == nil || ds.auth == nil || !ds.auth.VerifyToken(msg.AuthToken) {
