...
function renderSessionGraph(graph) {
  ...
  function renderTreeNode(key) {
    ...
    var childHtml = '';
    if (kids.length > 0) {
      childHtml = '<ul class="session-tree-children">' +
        kids.map(renderTreeNode).join('') + '</ul>';
    }
    return '<li class="session-tree-node">' +
      '<span class="session-tree-icon ' + iconClass + '">' + icon + '</span>' +
      '<span class="session-tree-label">' + escapeHtml(label) + '</span>' +
      '<span class="session-tree-meta">turns=' + n.turn_count + '</span>' +
      childHtml + '</li>';
  }

  var html = '<div class="card glass"><div class="card-title">Session Graph</div>' +
    '<ul class="session-tree">' + roots.map(renderTreeNode).join('') + '</ul></div>';
  el.innerHTML = html;
}
