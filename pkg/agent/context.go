...
func (cb *ContextBuilder) getIdentity(includeToolUseRule bool) string {
	...
	if includeToolUseRule {
		rules = append(
			...
			"**Task boards** - For composite workflows that call multiple agents or run multiple child steps, use task_board to create one stable board_id and planned steps. For serious workflows, include task_packet on the board root to capture objective, scope, acceptance criteria, verification plan, resources, reporting, and recovery policy; keep domain-specific fields under coding/media/research/nutrition. Pass that board_id, stable step_id values, readable step_title values, and depends_on to every delegate/spawn call. For synchronous delegate steps that may stall, set timeout_seconds explicitly. Use task_board results or task_status with board_id to inspect the workflow. Keep board state truthful: update local/manual/MCP-only steps when they finish, and never tell the user that work is still running or queued unless a real async handle exists (spawn, delegate, cron/job, async tool) or the next tool call will execute in the same turn.",
		)
	}
