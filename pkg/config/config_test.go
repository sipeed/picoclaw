...
func TestSaveConfig_PreservesExplicitDisabledAttachUserImages(t *testing.T) {
	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "config.json")

	cfg := DefaultConfig()
	cfg.Agents.Defaults.ImageInput.AttachUserImages = false
	...
	if !strings.Contains(string(data), `"attach_user_images": false`) {
		t.Fatalf("saved config should persist image_input.attach_user_images=false, got:\n%s", string(data))
	}
	...
	if loaded.Agents.Defaults.ImageInput.AttachUserImages {
		t.Fatal("image_input.attach_user_images should remain false after SaveConfig/LoadConfig round-trip")
	}
}
