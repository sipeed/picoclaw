...
企业微信智能机器人（AI Bot）是企业微信官方提供的 AI 对话接入方式，支持私聊与群聊，内置流式响应协议。Piconomous 当前同时支持两种接入模式：
...
- 当 `bot_id` 和 `secret` 同时存在时，Piconomous 会优先使用 WebSocket 长连接模式
- 否则，当 `token` 和 `encoding_aes_key` 同时存在时，Piconomous 会使用 Webhook 短连接模式
...
4. 在 Piconomous 配置文件中添加上述配置，重启 Piconomous
...
4. 将 Token 和 EncodingAESKey 填入 Piconomous 配置文件，启动服务后回到管理后台保存
...
用户发消息
  │
  ▼
Piconomous 立即返回 {finish: false}（Agent 开始处理）
  │
  ▼
...
若 Agent 处理时间超过轮询窗口，Piconomous 会：
...
配置 `processing_message` 后，当 Webhook 模式的流式轮询超时并切换到 `response_url` 主动推送模式时，Piconomous 会先返回这段提示语来结束当前流。
...
配置 `welcome_message` 后，当用户打开与 AI Bot 的聊天窗口时（`enter_chat` 事件），Piconomous 会自动回复该欢迎语。留空则静默忽略。
...
- 确认服务器防火墙已开放对应端口
- 检查 Piconomous 日志是否收到了来自企业微信的验证请求
