...
func spawnSubTurn(
	...
	// Shallow-copy the agent for SubTurn isolation.
...
// copyAgentInstance creates a shallow copy of an AgentInstance without copying
// the embedded instanceExt's sync.RWMutex, which would trigger go vet copylocks.
// The new instance gets a zero-valued instanceExt (fresh mutex).
func copyAgentInstance(src *AgentInstance) AgentInstance {
	return AgentInstance{
		ID: src.ID, Name: src.Name, Model: src.Model,
		Fallbacks: src.Fallbacks, Workspace: src.Workspace,
		MaxIterations: src.MaxIterations, TaskReminderInterval: src.TaskReminderInterval,
		MaxTokens: src.MaxTokens, Temperature: src.Temperature,
