...
# Utiliser le fournisseur Antigravity dans Piconomous

Ce guide explique comment configurer et utiliser le fournisseur **Antigravity** (Google Cloud Code Assist) dans Piconomous.
...
Si vous exécutez Piconomous sur un serveur (Coolify/Docker) et ne pouvez pas accéder à `localhost`, suivez ces étapes :
...
6.  **Collez-la dans le terminal** où Piconomous attend.

Piconomous extraira automatiquement le code d'autorisation et terminera le processus.
...
### Changer de modèle
Vous pouvez modifier le modèle par défaut dans `~/.piconomous/config.json` ou le remplacer via le CLI :
...
*   **429 Limite de débit** : Antigravity a des quotas stricts. Piconomous affichera le « temps de réinitialisation » dans le message d'erreur si vous atteignez une limite.
*   **404 Non trouvé** : Assurez-vous d'utiliser un ID de modèle provenant de la liste `piconomous auth models`. Utilisez l'ID court (par ex. `gemini-3-flash`) et non le chemin complet.
