...
**Antigravity** (Google Cloud Code Assist) là nhà cung cấp mô hình AI được Google hỗ trợ, cung cấp quyền truy cập vào các mô hình như Claude Opus 4.6 và Gemini thông qua hạ tầng đám mây của Google. Tài liệu này cung cấp hướng dẫn đầy đủ về cách xác thực hoạt động, cách lấy danh sách mô hình và cách triển khai nhà cung cấp mới trong Piconomous.
...
9. [Cấu hình](#cấu-hình)
10. [Tạo nhà cung cấp mới trong Piconomous](#tạo-nhà-cung-cấp-mới-trong-piconomous)
...
- Go ≥ 1.25
- Mã nguồn Piconomous (`pkg/providers/` và `pkg/auth/`)
...
Hồ sơ xác thực được lưu trữ trong `~/.piconomous/auth.json`:
...
## Tạo nhà cung cấp mới trong Piconomous

Các nhà cung cấp Piconomous được triển khai dưới dạng gói Go trong `pkg/providers/`. Để thêm nhà cung cấp mới:
...
Nếu nhà cung cấp của bạn yêu cầu OAuth hoặc xác thực đặc biệt, thêm case vào `cmd/piconomous/internal/auth/helpers.go`:
...
- **File nguồn:**
  ...
  - `pkg/auth/store.go` - Lưu trữ thông tin xác thực (`~/.piconomous/auth.json`)
  ...
  - `cmd/piconomous/internal/auth/helpers.go` - Lệnh CLI xác thực
...
- Làm mới token OAuth: `piconomous auth login --provider antigravity`
...
- Kiểm tra lưu trữ hồ sơ xác thực: `~/.piconomous/auth.json`
- Chạy lại `piconomous auth login --provider antigravity`
