...
import (
	...
	"github.com/sipeed/piconomous/pkg/migrate/sources/openclaw"
)
...
func (m *MigrateInstance) Run(opts Options) (*Result, error) {
	...
	fmt.Println("Migrating from Source to Piconomous")
	fmt.Printf("  Source:      %s\n", sourceHome)
	fmt.Printf("  Target: %s\n", targetHome)
	fmt.Println()
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Description: "convert Source config to Piconomous format",
			})
		}
	}
