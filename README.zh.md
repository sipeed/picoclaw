...
| **Zalo**             | ⭐⭐ 中等   | Webhook 模式（需要 HTTPS），适合本地 ngrok 调试 | 参见主 README 中的 Zalo(Webhook) 章节                                                                            |
...
* 选择一个强密码 `secret_token`（8–256 字符）。这是您自己的密钥，Zalo 会在 `X-Bot-Api-Secret-Token` 请求头中返回此值。
...
Zalo 需要 HTTPS webhook URL。对于本地开发，您可以使用 ngrok：
...
之后，在 Zalo 上向您的机器人发送消息，PicoClaw 将回复。
