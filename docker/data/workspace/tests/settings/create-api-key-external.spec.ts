...
test('Create API Key flow', async ({ page }) => {
  ...
  const description = 'Test External API Key';
  ...
  // Step 6: Select role External in Create API Key popup
  console.log('\n📍 Step 6: Select role External in Create API Key popup');
  const dialog = page.locator('.v-dialog--active, [role="dialog"]').last();
   ...
   * outside the dialog. We search globally for the list item with text "External".
   */
  const externalOption = page.locator('.v-overlay-container .v-list-item').filter({ hasText: /^External$/ }).first();
  await expect(externalOption).toBeVisible({ timeout: 5000 });
  await externalOption.click();
  await page.waitForTimeout(500);
  console.log('✅ PASS: Step 6 - Role External selected');
  ...
  // Step 8: Fill Description with Test External API Key
  console.log('\n📍 Step 8: Fill Description with Test External API Key');
  ...
  console.log('✅ Step 6: PASS - Role External selected');
  console.log('✅ Step 7: PASS - Expires in Days filled with 30');
  console.log('✅ Step 8: PASS - Description filled with Test External API Key');
  ...
  console.log('✅ Step 13: PASS - New API key appears in list with role External');
