...
// estimateArgsLen returns the approximate JSON-serialized length of a
// FunctionCall.Arguments map. Falls back to a key-count heuristic if
// marshaling fails.
func estimateArgsLen(args map[string]any) int {
	...
	return len(args) * 20 // rough heuristic
}
...
func estimateMessageTokens(msg providers.Message) int {
	...
	for _, tc := range msg.ToolCalls {
		...
		if tc.Function != nil {
			...
			chars += len(tc.Function.Name) + estimateArgsLen(tc.Function.Arguments)
		} else {
