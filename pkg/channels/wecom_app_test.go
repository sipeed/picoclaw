...
func TestNewWeComAppChannel(t *testing.T) {
	...
	t.Run("missing corp_id", func(t *testing.T) {
		...
		_, err := NewWeComAppChannel(cfg, config.MessagesConfig{}, msgBus)
		if err == nil {
	...
	t.Run("valid config", func(t *testing.T) {
		...
		ch, err := NewWeComAppChannel(cfg, config.MessagesConfig{}, msgBus)
		if err != nil {
...
func TestWeComAppChannelIsAllowed(t *testing.T) {
	...
	t.Run("empty allowlist allows all", func(t *testing.T) {
		...
		ch, _ := NewWeComAppChannel(cfg, config.MessagesConfig{}, msgBus)
...
func TestWeComAppVerifySignature(t *testing.T) {
	...
	t.Run("empty token skips verification", func(t *testing.T) {
		...
		chEmpty, _ := NewWeComAppChannel(cfgEmpty, config.MessagesConfig{}, msgBus)
...
<xml>
	...
	if msg.MsgId != 12345678 {
		t.Errorf("MsgId = %d, want %d", msg.MsgId, 12345678)
	}
	if msg.AgentID != 1000002 {
		t.Errorf("AgentID = %d, want %d", msg.AgentID, 1000002)
	}
